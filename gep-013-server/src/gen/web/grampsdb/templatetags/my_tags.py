...
def person_get_event(person, event_type):
    ...
    if index >= 0:
        ...
        return display_date(models.EventRef.objects.get(ref_object__handle=event_handle).ref_object)
    else:
        return ""
...
def make_name(name, user):
    if isinstance(name, models.Name):
        ...
        if user.is_authenticated():
            ...
            if libdjango.probably_alive(name.person.handle):
